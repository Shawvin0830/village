...
exports.InterviewPlannerSkill = void 0;
const common_1 = require("@nestjs/common");
const coze_coding_dev_sdk_1 = require("coze-coding-dev-sdk");
const supabase_client_1 = require("../storage/database/supabase-client");
let InterviewPlannerSkill = class InterviewPlannerSkill {
    get client() {
    ...
    async generate(topicId, subtopicId, requirements) {
        ...
        const { data: savedPlan, error } = await this.client
            ...
            .single();
        if (error)
            throw new Error(`保存采访策划失败: ${error.message}`);
        return {
            ...savedPlan,
    ...
    async refine(planId, feedback) {
        ...
        const { data: newPlan, error: insertError } = await this.client
            ...
            .single();
        if (insertError)
            throw new Error(`创建新版本失败: ${insertError.message}`);
        return {
            ...newPlan,
    ...
    async finalize(planId) {
        const { data, error } = await this.client
            ...
            .single();
        if (error)
            throw new Error(`确认定稿失败: ${error.message}`);
        return data;
    }
    async getByTopic(topicId) {
        ...
        if (error)
            throw new Error(`查询采访策划失败: ${error.message}`);
        return data || [];
    }
    ...
    async collectContext(topicId, subtopicId) {
...
| 🔁 传承 | Transmission | 谁接着做、还会不会继续 | 主题涉及手艺/习俗/方言时选 |`;
    ...
    async generatePlan(context, requirements) {
...
${requirements ? '9. **必须优先满足用户的具体要求**' : ''}`;
    ...
    async refinePlan(context, existingPlan, feedback) {
        const { topic } = context;
...
请根据用户反馈修改策划，返回完整的更新后策划（JSON格式）。`;
    ...
    async generateSupplementQuestions(context, existingPlan, requirements, existingCount) {
        const { topic } = context;
        const tipsData = existingPlan.tips;
        const existingCoreQuestions = tipsData?.core_questions || [];
...
exports.InterviewPlannerSkill = InterviewPlannerSkill;
exports.InterviewPlannerSkill = InterviewPlannerSkill = __decorate([
    (0, common_1.Injectable)()
], InterviewPlannerSkill);
//# sourceMappingURL=interview-planner.skill.js.map