/**
 * 转录整理师 Skill v3 — 故事档案馆
 *
 * 设计理念：你是一个有记忆的导游。每次采访是一块拼图，
 * 多次采访积累后拼出完整的祠堂故事地图。
 *
 * 核心能力：
 * 1. 故事分类：建筑史 / 工艺文化 / 图像寓意 / 人物传记 / 民俗风情 / 村落变迁
 * 2. 导览叙事：每条故事线输出一段可直接用于导览的可读文字
 * 3. 跨采访积累：新采访自动与已有故事线拼接融合
 * 4. 故事完整度：评估每条故事线的信息充分程度，指出缺什么
...
export type StoryCategory =
  ...
  | 'biography'           // 人物传记：与祠堂相关的人的生命故事
  | 'folk_custom'         // 民俗风情：祭祀、节庆、仪式、禁忌
  | 'village_change';     // 村落变迁：社区、宗族、经济、人口变化

const CATEGORY_META: Record<StoryCategory, { label: string; icon: string; description: string }> = {
...
export interface StoryFragment {
  ...
  dialect_original: string;        // 方言原话
  mandarin_text: string;           // 普通话转写
  summary: string;                 // 一两句概括
  flags: string[];                 // 标记（待核实/新发现/信息明确）
  time_range?: string;             // 仅音频模式
}
...
export interface GuidedNarrative {
  ...
  key_quote: string;               // 最具感染力的一句方言原话（作为导览亮点）
  visitor_hook: string;            // 一句话引子（吸引游客注意力的开场白）
  completeness: number;            // 0-100，信息充分度
  missing_pieces: string[];        // 还缺什么信息才能让故事更完整
}
...
export interface TimelineEvent {
  ...
  source_fragments: number[];      // 对应 fragments 数组下标
...
export interface Character {
  ...
  story: string;
  key_quotes: string[];
  related_story_threads: string[]; // 此人出现在哪些故事线中
...
export interface OrganizeResult {
  ...
  cross_references: string[];         // 跨故事线的关联发现
  next_interview_plan: string[];      // 下次采访建议（替代 deepening_suggestions，更具行动性）
}

// ─── 故事档案馆 HTML 模板（自包含，数据内联；占位符 __DATA_JSON__/__TITLE__/__SUBTITLE__/__NOTE__）───
...
export class TranscriptOrganizerSkill {
  /**
   * 单次 LLM 调用的输入字数上限。
   * 原因：整理时要回显 2× 原文（方言原文 + 普通话转写）再加分析，
   * 输出体量约为输入的 2~3 倍。文本过长会让输出超过模型最大 token 上限，
   * 导致 JSON 被截断、解析失败。超过此阈值时自动分段处理再合并。
   */
  private readonly SINGLE_CALL_LIMIT = 4000;
  ...
  async getStoryMap(topicId: string) {
    const { data: records, error } = await this.client
      ...
      .eq('topic_id', topicId)
      .eq('status', 'completed')
      .order('created_at', { ascending: true }); // 时间正序，后面的覆盖前面的

    if (error) throw new Error(`查询失败: ${error.message}`);

    // 按故事线聚合：后来的采访叙事覆盖/丰富早期的
    const narrativeMap = new Map<string, GuidedNarrative>();
    const allCharacters = new Map<string, Character>();
    const allTimeline: TimelineEvent[] = [];
    const allRelationships: Relationship[] = [];

    for (const r of records || []) {
      const analysis = r.ai_analysis as OrganizeResult;
      if (!analysis) continue;

      // 叙事：同名故事线取最新版（最新采访覆盖旧版）
   ...
   * 保存记录 + 更新故事线状态
   */
  private async saveRecordAndUpdateStoryThreads(
    topicId: string,
    opts: {
      subtopicId?: string;
      audioKey?: string;
    ...
    const matchedIds = analysis.fragments
      ...
      .filter((id): id is string => !!id && id !== subtopicId);
   ...
   * 核心：智能整理转写内容
   */
  private async organizeTranscript(
    topicId: string,
    text: string,
    opts: { hasAudio: boolean } = { hasAudio: false },
    ...
    // 已有故事线（从历史记录中提取）
    const existingNarratives: GuidedNarrative[] = [];
    const knownCharacters: string[] = [];
    for (const r of existingRecords || []) {
      const prev = r.ai_analysis as OrganizeResult;
      if (!prev) continue;
    ...
    // 已有故事线摘要（可叠加本次分段处理中已积累的故事线/人物，
    // 让后续段落归并到前面段落开出的故事线，避免同一故事线被拆成多条）
    const buildCtxStrings = (
      sessionNarratives: GuidedNarrative[] = [],
      ...
      const storyLineSummary = merged.length > 0
        ? merged.map((n) =>
            `【${CATEGORY_META[n.category]?.label || n.category}】${n.story_thread_name}（完整度${n.completeness}%）\n` +
            `  导览：${n.narrative.substring(0, 150)}...\n` +
            `  缺失：${n.missing_pieces.join('；') || '暂无'}`
          ).join('\n\n')
        : '暂无已有故事线（这是第一次采访）';
      const knownCharStr =
        [...new Set([...knownCharacters, ...sessionCharacters])].join('；') || '暂无';
...
- next_interview_plan 要具体、可执行，格式是"问谁""问什么""为了补全哪条故事线"`;
    ...
    // 处理单个文本块：把上下文（含本 session 已积累的故事线/人物）拼进 prompt，
    // 让后续段落能归并到前面段落开出的故事线
    const runChunk = async (
      ...
      const { storyLineSummary, knownCharStr } = buildCtxStrings(
        sessionNarratives,
        sessionCharacters,
      );
...
请进行专业整理。如果有已有故事线，务必将本次内容与对应故事线融合，更新导览叙事。`;
      ...
      try {
        const response = await llmClient.invoke(
          ...
          { temperature: 0.4 }, // 稍微提高一点，让叙事更有文采
        );
    ...
    const partials: OrganizeResult[] = [];
    const sessionNarratives: GuidedNarrative[] = [];
    const sessionCharacters: string[] = [];
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // 累积本 session 的故事线与人物，供后续段落归并（同名故事线取最新版）
      for (const n of res.narratives) {
        const idx = sessionNarratives.findIndex(
          (e) => e.story_thread_name === n.story_thread_name,
        );
        if (idx >= 0) sessionNarratives[idx] = n;
        else sessionNarratives.push(n);
      }
   ...
   * 解析 LLM 返回的 JSON
   */
  private parseLLMResponse(content: string): OrganizeResult {
    const jsonMatch =
    ...
    try {
      ...
      // 输出被截断时 JSON 不完整，尝试修复（截到最后一个平衡括号并补齐闭合）
  ...
  /**
   * 修复被截断的 JSON。
   * LLM 输出超过 token 上限时，返回的 JSON 往往在某个字符串或数组中间被硬截断，
   * 直接 JSON.parse 会失败。这里的策略：
   *   1. 从头扫描，跟踪转义 / 字符串状态与括号栈；
   *   2. 记录「最后一个安全截断点」——即在字符串外部出现逗号或闭合括号的位置，
   *      在这些点之前的内容一定是一个已经完整结束的元素 / 属性值；
   *   3. 把字符串截断到该安全点，去掉可能的尾随逗号；
   *   4. 按括号栈逆序补齐缺失的 ] / }；
   *   5. 尝试解析，成功返回对象，失败返回 null。
   */
  private salvageJson(raw: string): any | null {
    if (!raw) return null;

    const stack: string[] = []; // 记录尚未闭合的 { 或 [
    let inString = false;
    let escaped = false;
    let safeCutIndex = -1; // 最后一个安全截断点（该位置字符本身包含在内）
    ...
    for (let i = 0; i < raw.length; i++) {
      ...
      if (ch === '{' || ch === '[') {
        ...
        stack.pop();
        // 一个元素 / 对象刚刚完整闭合，是安全截断点
        safeCutIndex = i;
      } else if (ch === ',') {
        // 逗号说明前一个元素已完整结束，是安全截断点
        safeCutIndex = i;
      }
    }

    if (safeCutIndex < 0) return null;

    // 截断到安全点。若安全点是逗号则去掉它；否则保留（它是闭合括号）
    let truncated = raw.slice(0, safeCutIndex + 1);
    truncated = truncated.replace(/,\s*$/, '');

    // 重新扫描截断后的字符串，计算仍未闭合的括号并逆序补齐
    ...
    for (let i = 0; i < truncated.length; i++) {
      ...
      if (ch === '"') s = true;
      else if (ch === '{' || ch === '[') openStack.push(ch);
  ...
  private fallbackResult(
    text: string,
    subtopics?: Array<{ id: string; name: string; icon?: string }> | null,
  ): OrganizeResult {
    return {
      fragments: [
        {
  ...
  /**
   * 时间线去重：同 period 的合并 events
   */
  private deduplicateTimeline(timeline: TimelineEvent[]): TimelineEvent[] {
  ...
  /**
   * 关系去重：同 from+to+type 的只保留 detail 更长的
   */
  ...
  /**
   * 长文本分段：按自然边界（段落 → 句子）切分，绝不从句子中间硬切。
   * 每块尽量接近但不超过 maxChars。
   */
   ...
   * 合并多段分批整理的结果为一个完整结果。
   */
  private mergeChunkResults(results: OrganizeResult[]): OrganizeResult {
    ...
    for (const r of results) {
      ...
      for (const x of r.cross_references || []) crossSet.add(x);
      for (const x of r.next_interview_plan || []) planSet.add(x);
    }
  ...
  // ═══════════════════════════════════════════════════════
  //  故事档案馆 HTML 生成 + 分享（单文件自包含）
  // ═══════════════════════════════════════════════════════

  /**
   * 把一份 OrganizeResult 渲染成自包含的「故事档案馆」HTML。
   * 数据内联进页面，双击即可打开，无需服务器 / 不依赖 fetch。
   */
  private renderStoryArchiveHtml(
    ...
    const note = escHtml(
      meta.note || `累计 ${result.narratives?.length || 0} 条故事线持续积累中`,
    );
    // 防止内容中的 </script> 提前闭合脚本标签；\u003c 在 JSON 中等价于 <
  ...
  /**
   * 汇总某话题下全部已完成采访记录，合并成一份完整的累积 OrganizeResult。
   * 复用 mergeChunkResults：同名故事线取高完整度、人物按名合并累加提及次数、
   * 时间线/关系去重、跨线关联与采访计划去重。
   */
  private async aggregateTopic(topicId: string): Promise<OrganizeResult> {
    const { data: records, error } = await this.client
      .from('interview_records')
      .select('ai_analysis, created_at')
      .eq('topic_id', topicId)
    ...
    const analyses = (records || [])
      .map((r) => r.ai_analysis as OrganizeResult)
      .filter((a): a is OrganizeResult => !!a);
  ...
  /**
   * 上传 HTML 到对象存储，返回可分享的预签名 URL（失败返回 null，不阻断主流程）。
   */
  private async uploadHtml(html: string, name = 'story-archive'): Promise<string | null> {
    try {
      const storage = this.getStorage();
  ...
  /**
   * 【一步到位】输入文本 → 整理 → 存库 → 汇总该话题全部采访 → 渲染累积「故事档案馆」HTML 并上传分享。
   * 返回 { analysis(本次整理结果), merged(累积结果), html(自包含页面), url(分享链接/可能为null) }。
   */
  ...
  /**
   * 只渲染：把某话题已积累的全部采访汇总成故事档案馆 HTML（不新增采访）。
   */
  ...
  /**
   * 直接把一份 OrganizeResult 渲染成 HTML 字符串（不依赖数据库，便于本地预览/测试）。
   */
  renderArchiveFromResult(
    result: OrganizeResult,
    meta?: { title?: string; subtitle?: string; note?: string },
  ): string {
