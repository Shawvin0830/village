...
class WeappCI extends BaseCi_1.default {
    init() {
        ...
        try {
            this.ci = (0, npm_1.getNpmPkgSync)('miniprogram-ci', process.cwd());
        }
        catch (error) {
            printLog("error" /* processTypeEnum.ERROR */, chalk.red('请安装依赖：miniprogram-ci'));
            process.exit(1);
        }
    ...
    async open() {
        ...
        if (!(await fs.pathExists(ideStatusFile))) {
            printLog("error" /* processTypeEnum.ERROR */, errMesg);
            return;
        }
    ...
    async upload() {
        const { chalk, printLog, processTypeEnum } = this.ctx.helper;
        try {
            printLog("start" /* processTypeEnum.START */, '上传体验版代码到微信后台');
...
exports.default = WeappCI;
//# sourceMappingURL=WeappCI.js.map