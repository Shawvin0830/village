...
const icon_list_1 = require("./icon-list");
...
program
    ...
    if (options.list) {
        if (options.json) {
            console.log(JSON.stringify(icon_list_1.iconList.map(i => i.componentName), null, 2));
        }
        else {
            console.log(icon_list_1.iconList.map(i => i.componentName).join('\n'));
        }
        return;
    }
    ...
    const fuse = new fuse_js_1.default(icon_list_1.iconList, {
    ...
    const processQuery = (query) => {
        ...
        const exactMatch = icon_list_1.iconList.find(i => i.name.toLowerCase() === query.toLowerCase() ||
        ...
        if (query.length > 2) {
            const substringMatches = icon_list_1.iconList
                .filter(i => i.name
                .toLowerCase()
            ...
            output.forEach((item) => {
                if (item.exists) {
                    ...
                    if (item.suggestions.length > 0) {
                        console.log(`   Did you mean: ${item.suggestions.join(', ')}?`);
                    }
                }
            });
        }
